...
object UIUtils {
    ...
    fun vibrar(tipo: Vibracao) {
        ...
        val v: Vibrator = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
            (App.inst.getSystemService(Context.VIBRATOR_MANAGER_SERVICE) as VibratorManager).defaultVibrator
        } else {
            App.inst.getSystemService(Context.VIBRATOR_SERVICE) as Vibrator
        }
        
        when (tipo) {
            Vibracao.INTERACAO -> v.vibrate(VibrationEffect.createOneShot(50, VibrationEffect.DEFAULT_AMPLITUDE))
            Vibracao.SUCESSO   -> v.vibrate(VibrationEffect.createWaveform(longArrayOf(25, 25, 25, 25, 25, 25), VibrationEffect.DEFAULT_AMPLITUDE))
            Vibracao.ERRO      -> v.vibrate(VibrationEffect.createOneShot(200, VibrationEffect.DEFAULT_AMPLITUDE))
        }
    ...
    fun mostrarTeclado(view: View) {
        Handler().postDelayed({
                                  Handler(Looper.getMainLooper()).post {
                                      val motionEventDown: MotionEvent = MotionEvent.obtain(SystemClock.uptimeMillis(), SystemClock.uptimeMillis() + 100, MotionEvent.ACTION_DOWN, view.measuredWidth.toFloat(), (view.measuredHeight / 2).toFloat(), 0)
                                      view.dispatchTouchEvent(motionEventDown)
                                      motionEventDown.recycle()
                                      val motionEventUP: MotionEvent = MotionEvent.obtain(SystemClock.uptimeMillis() + 200, SystemClock.uptimeMillis() + 300, MotionEvent.ACTION_UP, view.measuredWidth.toFloat(), (view.measuredHeight / 2).toFloat(), 0)
